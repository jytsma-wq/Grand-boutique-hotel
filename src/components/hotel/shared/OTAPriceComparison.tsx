...
export default function OTAPriceComparison({ 
  ...
  const t = useTranslations('rooms.priceComparison');
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-forest-700">{t('title')}</span>
      </div>

      <div className="grid grid-cols-2 gap-4 mb-3">
        <div className="text-center">
          <div className="text-xs text-forest-500 uppercase tracking-wide mb-1">{t('otaPrice')}</div>
          <div className="text-lg text-forest-400 line-through">
            ${otaPrice.toLocaleString()}
          </div>
        </div>
        <div className="text-center">
          <div className="text-xs text-brass-600 uppercase tracking-wide mb-1">{t('directPrice')}</div>
      ...
      <div className="flex items-center justify-center gap-2 bg-forest-100 rounded-full py-2 px-4">
        ...
        <span className="text-sm font-medium text-forest-700">
          {t('save', { amount: savings, percent: savingsPercent })}
        </span>
      </div>
